...
rm(list = ls(all = TRUE))
 ...
 # source("/data3/stk05236/uebergabe/kidneyGPS2.4_underdevelopment/run_before_app_server.r")
# get the directoy if you source the pre-processing script
sourceDir <- dirname(sys.frame(1)$ofile)

print(paste(date(),"Loading files .."))
all_sig=fread("/data3/stk05236/lmm/03_eval/metal_eGFR_meta1.TBL.Indep.500k.5e8.indep_egfr_lowering.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
indepX=fread("/data3/stk05236/lmm/03_eval/metal_eGFR_meta1.TBL.Indep.500k.5e8.indepX_edited_MHC-tw.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
cred_var=read.table("/data3/stk05236/clean_GPS/6_7_NK/05_cred_var/all_cred_var_99.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
locus_table=read.table("/data3/stk05236/clean_GPS/6_7_NK/04_locus_table/locus_table_cys_bun.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
signal_table=read.table("/data3/stk05236/clean_GPS/6_7_NK/06_signal_table/signal_table_cred_var.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
CADD=read.table("/data3/stk05236/clean_GPS/6_7_NK/10_protein_altering/results_2023-05-22/cadd_in_cred_var.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)

# add here the new files, first glomerulus and then tubulus
NEPTUNE_glo=fread("/data3/stk05236/uebergabe/kidneyGPS2.4_underdevelopment/data_update/glomerular/cred_var_with_sampsonQTL_glomerular.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
NEPTUNE_tub=fread("/data3/stk05236/uebergabe/kidneyGPS2.4_underdevelopment/data_update/glomerular/cred_var_with_sampsonQTL_tubular.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

susztak_glo=fread("/data3/stk05236/clean_GPS/6_7_NK/08_eQTLs/susztak/cred_var_with_nephQTL_glomerular.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
susztak_tub=fread("/data3/stk05236/clean_GPS/6_7_NK/08_eQTLs/susztak/cred_var_with_nephQTL_tubulointerstitial.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)


GTEx_eQTL=fread("/data3/stk05236/clean_GPS/6_7_NK/08_eQTLs/gtex/all_cred_var_with_gtex.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)
GTEx_sQTL=fread("/data3/stk05236/clean_GPS/6_7_NK/09_sQTLs/gtex/all_cred_var_with_gtex.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)


MGI=read.table( "/data3/stk05236/MGI_lookup/genes_and_pheno/MGI_table_kidney.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE, fill=TRUE, quote="",comment="") #raw data
OMIM=read.table("/data3/stk05236/OMIM/OMIM_Groopman_combined/all_kidney_genes_sorted_nearly_without_duplicates_ks.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE) #raw data improved
ADTKD=read.table("/data3/stk05236/wiesener/all_genes_with_classification.txt", sep = "\t", header = T, stringsAsFactors = F) # raw data from manuscript

# coloc_tub=read.table( "/data3/stk05236/coloc/results_T/coloc_tubulo.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
# coloc_glo=read.table( "/data3/stk05236/coloc/results_G/coloc_glomerulus.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
# now read it from a project folder (not from stks private drive - th enew version
coloc_tub=read.table( "/data3/stk05236/uebergabe/second_transformation/14_coloc/coloc_tubulo.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
coloc_glo=read.table( "/data3/stk05236/uebergabe/second_transformation/14_coloc/coloc_glomerulus.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)

GPS=read.table("/data3/stk05236/clean_GPS/6_7_NK/07_GPS/table_for_GPS_signal_based.txt", header=TRUE, sep = "\t", stringsAsFactors=FALSE)
DM_decline = read.table("/data3/stk05236/GPS_App/DM_decline_effects_withStanzickId_mg_clean.txt", header=TRUE, sep = "\t", stringsAsFactors=FALSE) # manually curated
DM_stats = read.table("/data3/stk05236/clean_GPS/6_7_NK/12_DM/DM_and_noDM_statistics_for_cred_vars.txt", header=TRUE, sep = "\t", stringsAsFactors=FALSE)
drugability = read.table("/data3/stk05236/clean_GPS/6_7_NK/11_drugability/targets_drugs_diseases_in_GPS_marked_kidney_drugs.txt", header=TRUE, sep = "\t", stringsAsFactors=FALSE, comment="", quote="",fill=TRUE)



### Gene Name Reference
genes = fread( "/data3/stk05236/clean_GPS/6_7_NK/inputs/glist.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

genes_sorted <- genes[order(genes$Chr, genes$Pos1), ]

real_genes = fread( "/data3/stk05236/clean_GPS/6_7_NK/inputs/genes_an_ids.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE,  data.table = FALSE)

print(paste(date(),"File loading finished"))
...
MIM_number=read.table("/data3/stk05236/OMIM/OMIM_gene_pheno/genemap2_edited.txt", header = TRUE,  sep="\t", stringsAsFactors = FALSE, quote="", fill=TRUE)
...
#files for locuszoom
#lz_files=list.files("/data3/stk05236/GPS_App/App_test/www")
#lz_files=lz_files[-1]
#locus_id_files=c()
...
### prepare coloc
#coloc_tub=read.table( "/data3/stk05236/lmm/coloc/coloc_tubulo.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)

#coloc_glo=read.table( "/data3/stk05236/lmm/coloc/coloc_glomerulus.txt",  header = TRUE, sep = "\t", stringsAsFactors = FALSE)
...
print(paste(date(),"file preparation finished"))

print(paste(date(),"Processing GPS"))
### generate GPS-table
for(i in 1:nrow(GPS)){
  ...
  if(!i%%2000)print(paste(date(),"Done",i,"of",nrow(GPS)))
  }
...
print(paste(date(),"Processing GPS_10"))
...
for(i in 1:nrow(GPS_10)){
  ...
  if(!i%%2000)print(paste(date(),"Done",i,"of",nrow(GPS_10)))
  }


if(FALSE){
	# START: would this as an alternive workflow be possible?:
	library(dplyr)

	# Define a helper function to count unique rsids based on conditions
	count_unique_rsid <- function(df, gene_col, signal_col, region_col, ppa_col, threshold, score_col = NULL, score_thresh = NULL) {
	  if (!is.null(score_col) & !is.null(score_thresh)) {
		df <- df %>% filter(!!sym(ppa_col) > threshold, !!sym(score_col) > score_thresh)
	  } else {
		df <- df %>% filter(!!sym(ppa_col) > threshold)
	  }
	  df %>%
		group_by(!!sym(gene_col), !!sym(signal_col), !!sym(region_col)) %>%
		summarize(count = n_distinct(RSID), .groups = "drop")
	}

	# Compute unique RSID counts per condition
	glo_count <- count_unique_rsid(glo, "Approved.symbol", "signal_id", "region_id", "ppa", 0.1)
	tub_count <- count_unique_rsid(tub, "Approved.symbol", "signal_id", "region_id", "ppa", 0.1)
	GTEx_kidney_count <- count_unique_rsid(GTEx_eQTL, "gene", "signal_id", "region_id", "ppa", 0.1)
	GTEx_other_count <- count_unique_rsid(gtex_without_kidney_show, "`Affected gene`", "signal_id", "region_id", "PPA", 0.1)
	GTEx_sQTL_kidney_count <- count_unique_rsid(GTEx_sQTL, "gene", "signal_id", "region_id", "ppa", 0.1)
	GTEx_sQTL_other_count <- count_unique_rsid(sqtl_without_kidney_show, "`Affected gene`", "signal_id", "region_id", "PPA", 0.1)

	# Compute CADD counts with additional filtering
	CADD_stop <- count_unique_rsid(CADD, "GeneName", "signal_id", "region_id", "PPA", 0.1, "ConsScore", 6)
	CADD_synonymous <- count_unique_rsid(CADD, "GeneName", "signal_id", "region_id", "PPA", 0.1, "ConsScore", 4)
	CADD_other <- count_unique_rsid(CADD, "GeneName", "signal_id", "region_id", "PPA", 0.1, "ConsScore", 0)

	# Merge counts into GPS_10 using left_join
	GPS_10 <- GPS %>%
	  left_join(glo_count, by = c("Gene" = "Approved.symbol", "signal_id", "locus_id")) %>%
	  rename(NEPTUNE_glomerulus = count) %>%
	  left_join(tub_count, by = c("Gene" = "Approved.symbol", "signal_id", "locus_id")) %>%
	  rename(NEPTUNE_tubulointerstitium = count) %>%
	  left_join(GTEx_kidney_count, by = c("Gene" = "gene", "signal_id", "locus_id")) %>%
	  rename(GTEX_eQTL_kidney_tissue = count) %>%
	  left_join(GTEx_other_count, by = c("Gene" = "`Affected gene`", "signal_id", "locus_id")) %>%
	  rename(GTEX_eQTL_any_other_tissue = count) %>%
	  left_join(GTEx_sQTL_kidney_count, by = c("Gene" = "gene", "signal_id", "locus_id")) %>%
	  rename(GTEX_sQTL_kidney_tissue = count) %>%
	  left_join(GTEx_sQTL_other_count, by = c("Gene" = "`Affected gene`", "signal_id", "locus_id")) %>%
	  rename(GTEX_sQTL_any_other_tissue = count) %>%
	  left_join(CADD_stop, by = c("Gene" = "GeneName", "signal_id", "locus_id")) %>%
	  rename(stop_gained_stop_lost_non_synonymous = count) %>%
	  left_join(CADD_synonymous, by = c("Gene" = "GeneName", "signal_id", "locus_id")) %>%
	  rename(canonical_splice_noncoding_change_synonymous_splice_site = count) %>%
	  left_join(CADD_other, by = c("Gene" = "GeneName", "signal_id", "locus_id")) %>%
	  rename(Cadd15_other = count)

	# Replace NA values with 0
	GPS_10[is.na(GPS_10)] <- 0
	# STOP: would this as an alternive workflow be possible?:
}

print(paste(date(),"Processing GPS_50"))
...
for(i in 1:nrow(GPS_50)){
  ...
  if(!i%%2000)print(paste(date(),"Done",i,"of",nrow(GPS_50)))
  }
...
print(paste(date(),"Mathias: Integrating the decline loci also from Wiegrebe et al. to the workspace file"))

# make this for each signal separately
GPS_show[which(GPS_show$"Locus name**"=="[UMOD-PDILT]" & GPS_show$"Signal ID" %in% c("Signal 1","Signal 2","Signal 4")),"decline_effect"] <- "decline"
GPS_10_show[which(GPS_10_show$"Locus name**"=="[UMOD-PDILT]" & GPS_10_show$"Signal ID" %in% c("Signal 1","Signal 2","Signal 4")),"decline_effect"] <- "decline"
GPS_50_show[which(GPS_50_show$"Locus name**"=="[UMOD-PDILT]" & GPS_50_show$"Signal ID" %in% c("Signal 1","Signal 2","Signal 4")),"decline_effect"] <- "decline"

# # TPPP: here is nothing to do
# GPS_show[which(GPS_show$"Locus name**"=="[TPPP]" ),c("Locus name**","Locus ID","Signal ID","DM_effect","decline_effect")]

# C15orf54: nothing to do, because not part of the Stanzick loci

# OVOL1: nothing to do, because already part of the signals here

# PRKAG2: nothing to do, because already part of the signals here

# FGF5: nothing to do, because already part of the signals here
 
# SDCCAG8: here add the first signal to the list
GPS_show[which(GPS_show$"Locus name**"=="[SDCCAG8]" & GPS_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_10_show[which(GPS_10_show$"Locus name**"=="[SDCCAG8]" & GPS_10_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_50_show[which(GPS_50_show$"Locus name**"=="[SDCCAG8]" & GPS_50_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"

# RRAGD: add the first signal
GPS_show[which(GPS_show$"Locus name**"=="[RRAGD]" & GPS_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_10_show[which(GPS_10_show$"Locus name**"=="[RRAGD]" & GPS_10_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_50_show[which(GPS_50_show$"Locus name**"=="[RRAGD]" & GPS_50_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"

# GGT7: add the first signal
GPS_show[which(GPS_show$"Locus name**"=="[GGT7]" & GPS_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_10_show[which(GPS_10_show$"Locus name**"=="[GGT7]" & GPS_10_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"
GPS_50_show[which(GPS_50_show$"Locus name**"=="[GGT7]" & GPS_50_show$"Signal ID" %in% c("Signal 1")),"decline_effect"] <- "decline"

# PRAG1 or better :LOC157273
GPS_show[which(GPS_show$"Locus name**"=="[LOC157273]" & GPS_show$"Signal ID" %in% c("Signal 2")),"decline_effect"] <- "decline"
GPS_10_show[which(GPS_10_show$"Locus name**"=="[LOC157273]" & GPS_10_show$"Signal ID" %in% c("Signal 2")),"decline_effect"] <- "decline"
GPS_50_show[which(GPS_50_show$"Locus name**"=="[LOC157273]" & GPS_50_show$"Signal ID" %in% c("Signal 2")),"decline_effect"] <- "decline"

print(paste(date(),"Writing out final workspace file"))

workspace_image=paste(sourceDir,"data/workspace_mg.RData", sep="/")
save.image(file=workspace_image)
print(paste(date(),"Done. Exiting"))
